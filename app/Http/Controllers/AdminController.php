...
class AdminController extends Controller
{
    ...
    public function update_product(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $file_name = $current_timestamp . '.' . $file_extension;
        ...
        if ($request->hasFile('images')) {
            ...
            foreach ($files as $index => $file) {
                ...
                if (in_array($extension, $allowedfileExtensions)) {
                    $file_name = $current_timestamp . '-' . ($index + 1) . '.' . $extension;
                    $path = $file->storeAs('products', $file_name, 'public_uploads');
                    $gallery_arr[] = $path;
                }
            }

            // Delete old gallery images if applicable
            if ($product->images) {
                $old_gallery_images = explode(', ', $product->images);
                foreach ($old_gallery_images as $old_image) {
                    if (Storage::disk('public_uploads')->exists($old_image)) {
